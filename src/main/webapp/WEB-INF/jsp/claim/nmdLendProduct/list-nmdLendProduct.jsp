...
<%@include file="../../common/navbar.jspf" %>
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="nmdLendProduct">
			<tr>
				<td><a href="nmdLendProduct/form?id=${nmdLendProduct.id}">${nmdLendProduct.name}</a></td>
				<td>${nmdLendProduct.remarks}</td>
				<td>
    				<a href="nmdLendProduct/form?id=${nmdLendProduct.id}">修改</a>
					<a href="nmdLendProduct/delete?id=${nmdLendProduct.id}" onclick="return confirm('确认要删除该记录吗？', this.href)">删除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
