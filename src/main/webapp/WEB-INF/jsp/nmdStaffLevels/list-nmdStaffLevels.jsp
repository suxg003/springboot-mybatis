...
<%@include file="../common/navbar.jspf" %>
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="nmdStaffLevels">
			<tr>
				<td><a href="nmdStaffLevels/form?id=${nmdStaffLevels.id}">${nmdStaffLevels.name}</a></td>
				<td>${nmdStaffLevels.remarks}</td>
				<td>
    				<a href="nmdStaffLevels/form?id=${nmdStaffLevels.id}">修改</a>
					<a href="nmdStaffLevels/delete?id=${nmdStaffLevels.id}" onclick="return confirm('确认要删除该记录吗？', this.href)">删除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
