...
function showINvestRecordsTable(path){
    ...
    $('#depositRecordsTable').dataTable({
        ...
        "sAjaxSource": "/fund/deposit/records",
...
$('#downloaddeposit').on('click',function (){

    var $dateRange=$('#date-range-picker').val();
    var path = $('select[name=payPath]').val();
    var searchStr = $('#depositRecordsTable_filter input').val();
    var href = '/fund/deposit/down?startDate=' + $dateRange.split(' - ')[0]+'&endDate='+$dateRange.split(' - ')[1]+'&payPath='+path+'&sSearch='+searchStr;
