...
var newPlan = {
    ...
    init: function () {
      ...
      //  self.renderCreditList();
        ...
        $('#addPlanBtn').click(function () {
        	if (checkNewPlan()) {
	        	$("#opType").val('open');
	        	$("#rate").val(accMul($("#rate").val(), 100));
	        	$("#rateAdd").val(accMul($("#rateAdd").val(), 100));
	        	$("#newPlanForm").submit();
        	} else {
        		return false;
        	}
    ...
    getRepaymentDate: function (repaymentDate) {
        $.get("/uplan/computeDate/" + repaymentDate,
	        function (data) {
	            ...
	            $('input[name=investEndDate]').val(moment(Number(data)).format("YYYY-MM-DD"));
	        }
        );
    },
    renderCreditList: function () {
        ...
        if ($.trim(creditList) || creditList.length > 0) {
            ...
            $('#maxAmount').attr('value', 0.00);
        	 $('#rate').val("");
             $('#repaymentDate').val("");
        }
    },
    renderCreditListInit: function () {
        var self = this;
        creditList = self.defaultData.jqPlanEl.val();
        var checkOld = "";
        $('#creditInfoList').dataTable({
            ...
            "fnDrawCallback": function (oSettings) {
                $("#searchRechargeHistory").html('查询');
                $("#searchRechargeHistory").prop('disabled', false);
                $('#creditInfoList').attr('data-checkedUplanId', checkOld);
                newPlan.defaultData.jqPlanEl.val(checkOld);
            },
            "sAjaxSource": "/uplan/getLoansOld",
            "fnServerData": function (sSource, aoData, fnCallback, oSettings) {
                oSettings.jqXHR = $.ajax({
                    "dataType": 'json',
                    "type": "GET",
...
function checkInit() {
	// 产品类型
	$('#productType').val($('#productTypeStr').val());
	// 还款方式
	$('#repaymentType').val($('#repaymentTypeStr').val());
	// 保障类型
	$('#guaranteeType').val($('#guaranteeTypeStr').val());
	// 到期退出方式
	$('#expireOutType').val($('#expireOutTypeStr').val());
	// 提前退出方式
	$('#aheadOutType').val($('#aheadOutTypeStr').val());
	// 收益率
	$('#loanRate').val($('#rate').val());
	// 还款日期
	$('#loanEndDate').val($('#repaymentDate').val());
	// 是否为新手标
	if ($('#freshPlanStr').val() == "true") {
		$('#freshPlan').val("1");
	} else {
		$('#freshPlan').val("0");
	}
