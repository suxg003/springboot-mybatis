...
Func.prototype.init = function () {
    ...
    $('#btn_today').on('click',function(){
    	$('#reservation').val(moment().format("YYYY-MM-DD") + ' - ' + moment().format("YYYY-MM-DD"));
    	self.loadData();
    	$('#btn_beforeToday').removeClass('btn-blue');
    	$('#btn_today').addClass('btn-blue');
    });
    
    $('#btn_beforeToday').on('click',function(){
    	getBeforeToday();
    });
    
    //默认显示今天以前
    getBeforeToday();
    function getBeforeToday(){
    	$('#reservation').val(moment().subtract('year', 100).format("YYYY-MM-DD") + ' - ' + moment().subtract('days', 1).format("YYYY-MM-DD"));
    	$('#btn_today').removeClass('btn-blue');
    	$('#btn_beforeToday').addClass('btn-blue');
    	self.loadData();
    }
...
Func.prototype.loadData = function () {
    ...
    $('#reApRedeemTable').dataTable({
        ...
        "aoColumns": [
            ...
            {"mData": "repayTypeKey", "sDefaultContent": ""},//8
            {"mData": "fee", "sDefaultContent": ""},//9
            {"mData": "receivableAmount", "sDefaultContent": ""},//10
            {"mData": "applyStatusKey", "sDefaultContent": ""},//11
            {"mData": "id", "sDefaultContent": ""}//12
