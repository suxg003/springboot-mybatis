...
$('#saveFileBtn').on('click', function () {
    ...
    $.each(files, function (index, file) {
        ...
        $.ajax({
            url: '/claimLoan/importFileCurrentLoan',
            data: formData,
            processData: false,
            contentType: false,
            type: 'POST',
...
function curClaimList() {
    ...
    $('#curClaimDataTable').dataTable({
        ...
        fnCreatedRow: function(nRow, aData, iDataIndex) {
            ...
            if(isOpen){
                openHtml = ' <a class="btn btn-xs btn-info bg-palegreen" title="启用" onclick="showConfirm(\'确定要启用么？\',\'/claimLoan/currentRelease?loanId='+id+'\')">\
                      <i class="fa fa-check"></i>启用\
                    </a> ';
            }else{
                openHtml = ' <a class="btn btn-danger btn-xs" title="禁用" onclick="showConfirm(\'确定要禁用么？\',\'/claimLoan/currentDisable?loanId='+id+'\')">\
        ...
        "sAjaxSource": "/claimLoan/currentLoanList",
...
function allHandler(type){
    ...
    isSync = true;
    if(type == "open"){
        turl = '/claimLoan/currentRelease?loanId='+ids;
    }else{
        turl = '/claimLoan/currentDisable?loanId='+ids;
    }
