...
//FIXED: for sidebar => nav-list scroll when overflow (2015-04-13/xjf)
function forceNavScroll() {
    $('.nav.nav-list').css('max-height', $(window).height() - $('#sidebar-shortcuts').height() - $('.navbar.navbar-inverse.navbar-fixed-top').height());
    $('.nav.nav-list').css('overflow', 'auto');
}
$(function(){
    forceNavScroll();
    $(window).bind('resize', forceNavScroll);
});